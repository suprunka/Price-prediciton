...
#szóste
# groupByGrade = housing[['yr_renovated', 'price']].groupby(['yr_renovated']).count()
# groupByGrade['index'] = groupByGrade.index
# numberOfNotRenovated = groupByGrade.where(groupByGrade['index'] == 0)['price'].sum().sum()
# numberOfRenovated = groupByGrade.where(groupByGrade['index'] > 0)['price'].sum().sum()
...
p7 = figure(plot_width=1000, plot_height=1000, x_axis_type='datetime')
p7.yaxis.formatter = BasicTickFormatter(use_scientific=False)

p7.line(groupByPrice['index'], groupByPrice['price'], line_width=2, color="#33A02C",
        legend='Earned money')
p7.add_tools(HoverTool(tooltips=[("Date", "@x{DateTime}"), ('Sum from sold houses', '@y{int}')]))
show(p7)
